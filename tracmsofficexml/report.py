...
class ExcelReportModule(Component):
    ...
    def pre_process_request(self, req, handler):
        if re.match(r'/report/[0-9]+', req.path_info) \
            ...
            orig = handler.execute_paginated_report
            def execute_report(req, db, id, sql, args, limit=0, offset=0):
                del handler.execute_paginated_report
                return orig.__call__(handler, req, db, id, sql, args, 1000000, 0)
            handler.execute_paginated_report = execute_report
        return handler
